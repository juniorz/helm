...
func (c *ChartDownloader) Fetch(ref, version string) (string, *provenance.Verification, error) {
	...
	return c.provenanceVerificationPolicy(chartPath)
}
...
func (c *ChartDownloader) provenanceVerificationPolicy(chartPath string) (string, *provenance.Verification, error) {
	// Nothing to verify
	if c.Verify == VerifyNever || c.Verify == VerifyLater {
		return chartPath, &provenance.Verification{}, nil
	}
	...
	ver, err := VerifyChart(chartPath, c.Keyring)
...
// Downloaded files are stored in the cache directory.
func (c *ChartDownloader) downloadChartFiles(ref, version string) (string, error) {
	...
	// chart cache is the workspace for chart downloads
...
func (c *ChartDownloader) resolveChartVersion(ref, version string) (*url.URL, *repo.Entry, error) {
	...
	// rc === r.Config
	// ??? Why not use simply use r.Client to fetch all charts from that repo, instead of copying configs from one client to another?
